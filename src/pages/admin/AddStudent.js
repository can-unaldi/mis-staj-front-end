...
const AddStudent = ({ redirectTo = "/home" }) => {
  const intl = useIntl();
    ...
    const getUser = async () => {
      try {
        const responseData = await sendRequest(
          `${process.env.REACT_APP_BACKEND_URL}/users/advisors`
        );
        setPathValue("advisor", responseData.advisors[0].id);
      ...
      try {
        const responseData = await sendRequest(
          ...
          JSON.stringify({
            ...
            password: formData.studentNumber,
            type: 0,
        ...
        setSnackbar({
          open: true,
          status: true,
          message: "Öğrenci başarı ile oluşturuldu.",
        });
        setTimeout(function () {
          navigate("/home");
        }, 1000);
    ...
    >
      ...
      >
        <Typography variant="h6" style={{ marginTop: 20 }}>
          {intl.formatMessage({ id: "AddStudent" })}
        </Typography>
