...
const UpdateUser = ({ redirectTo = "/home" }) => {
  const intl = useIntl();
    ...
    const getUser = async () => {
      try {
        const responseData = await sendRequest(
          `${process.env.REACT_APP_BACKEND_URL}/users/details/${userId}`
        );
        setPathValue("name", responseData.user.name);
        setPathValue("email", responseData.user.email);
        setPathValue("type", responseData.user.type);
    ...
    >
      ...
      >
        <Typography variant="h6" style={{ marginTop: 20 }}>
          {intl.formatMessage({ id: "UpdateUser" })}
        </Typography>
